...
class MyToDoDoneView(TemplateResponseMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            todo_done_list = MyToDoDone.objects.filter(user=request.user)
            ...
            pretty_good_count = DailyEmotion.objects.filter(mood='Pretty Good', date__gte=seven_days_ago, user=request.user).count()
            awful_count = DailyEmotion.objects.filter(mood='Awful', date__gte=seven_days_ago, user=request.user).count()
            return self.render_to_response({'todo_done_list': todo_done_list, 'happy_count': happy_count,
                                            'ok_count': ok_count, 'sad_count': sad_count, 'pretty_good_count':pretty_good_count,
...
class MyDiaryView(TemplateResponseMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        my_diaries = Diary.objects.filter(user=request.user)#[:5]
